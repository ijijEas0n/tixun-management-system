export function getTencentVoiceErrorCode(error: unknown) {
  if (!error || typeof error !== 'object') return null;
  const details = error as { code?: unknown };
  return details.code === undefined || details.code === null ? null : String(details.code);
}

export function isTencentVoiceCloseCode(error: unknown, code: number) {
  return getTencentVoiceErrorCode(error) === String(code);
}
...
export function formatTencentVoiceError(error: unknown) {
  ...
  if (error && typeof error === 'object') {
    ...
    if (isTencentVoiceCloseCode(error, 1006)) {
